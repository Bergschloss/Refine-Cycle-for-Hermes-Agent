...
| O-25 | Full live cycle (dry_run → apply → `/refine audit` → rollback → confirm gone) on current code | UNPROVEN | reviewer returned `llm_incomplete`/`malformed` (10 output tokens) because the active model `openrouter/free` (host route, pin removed) is too weak for structured JSON — **model-choice issue, not a plugin defect**. No proposal produced, so the apply/rollback chain was not exercised | 2026-08-25 |
