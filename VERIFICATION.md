...
≥ Note: round-14 `PROVEN` rows E-01..E-06 (and the seeded O-01..O-19 reusing that report's data) were proven **on the 08-14 codebase**. They are **superseded** on 2026-08-25: "proven on 08-14 code" and "proven on current code" are different claims. Do not delete them; their E-rows now carry the current-code Part-1 verdict below.
...
| O-25 | Full live cycle (dry_run → apply → `/refine audit` → rollback → confirm gone) on current code | UNPROVEN | two live dry-runs on current code (`c11712f`, pin removed): (a) host route `openrouter/free` → reviewer `llm_incomplete`/`malformed` (10 output tokens, too weak for structured JSON); (b) current session route `deepseek-v4-flash-vision-exp @ opencode-go` → reviewer `llm_route_error` (latency 769 ms, structured call unavailable). Neither produced a proposal, so the apply/rollback chain was not exercised — **model-choice issue, not a plugin defect** | 2026-08-25 |
