...
def _parse_prompt_note_rule(content):
    ...
    if m:
        ...
        # A bare name can be both a host tool and a shell binary (`terminal`,
        # `patch`). The host's process registry cannot decide which one this turn
        # means: `agent.valid_tool_names` is filtered by enabled/disabled toolsets,
        # availability check_fns, dynamic schemas and tool-search assembly, while
        # pre_llm_call hooks receive none of that effective set. Querying the
        # registry made a disabled `patch` look callable and moved its rule from
        # the binary door to an unreachable tool door. Until Hermes exposes this
        # turn's effective names to the hook, keep the pre-existing deterministic
        # shape classification; do not approximate invocation state from process
        # registration.
...
# A tool name as it may be enforced. One-character executable identities are
# included because the action side also protects real CLIs such as ``R``; the
# stopword gate below keeps grammatical one-letter heads (``a``, ``i``) out.
# Punctuation covers real names (``mcp__server__tool``, ``namespace:tool``,
# ``python3.11``).
...
def _tool_identity(word):
    """A note's word as an enforceable target identity, or "" if it is not one.
    ...
    identity vocabulary on purpose. Conditions use it for tool names; action
    tails may also name a CLI (including a one-character executable such as R).
    The grammar differs, but answering "is this one bounded identity rather than
    prose?" twice is how the two sides drift.
    """
...
def _binary_matches(binary: str, target: str) -> bool:
    ...
    if binary == target:
        return True
    # One-character executable identities (notably R) were always valid input to
    # this matcher, but only became reachable from reroute notes once the action
    # identity gate stopped requiring two characters. The legacy boundary rules
    # below deliberately allow numeric and punctuation boundaries for names like
    # python3.11; applied to target ``r`` they also absorb unrelated executables
    # (r2, 2r, r-foo). A one-character name has no safe fuzzy boundary: after
    # `_extract_binaries` has already normalized basename, case and `.exe`, it is
    # exact or it is a different executable.
    if len(target) == 1:
        return False
    if target in binary:
