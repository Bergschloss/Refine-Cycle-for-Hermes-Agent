...
# Subagent proposer. ``_subagent_lifecycle_provider`` is installed by
# ``register()`` once the host plugin context exists; it stays None in bare
# imports (tests, offline runs), which forces the structured fallback.
_subagent_lifecycle_provider: Optional[Any] = None
# Subagent ids of children the plugin launched as proposers. The pre_tool_call
# hook refuses write tools (skill_manage) for these children's sessions.
_PROPOSER_SUBAGENT_IDS: set = set()
_PROPOSER_CORRELATION_PREFIX = "refine-proposer-"
...
def _set_subagent_lifecycle_provider(provider: Optional[Any]) -> None:
    """Install the host's plugin-safe subagent lifecycle service accessor."""
    global _subagent_lifecycle_provider
...
_PROPOSER_GOAL = (
    "You are the refine proposer: one pass of a self-improvement cycle. "
    "Your parent gathered repeated failures and user corrections from the "
    "agent's own session history and renders them below. Propose ONE minimal "
    "edit to the agent's durable context (a skill, a memory entry, or a "
    "prompt note) that prevents the most repeated failure.\n\n"
    "MANDATORY verification before proposing:\n"
    "1. Run skills_list() and read every name and description.\n"
    "2. Use skill_view() to OPEN any skill whose name or description could "
    "plausibly cover this lesson — including the skill's linked files when "
    "they exist.\n"
    "3. If a rule that covers the failure already exists — even worded "
    "differently — return no_op with a reason that quotes the rule and names "
    "the skill and the section/line where you found it.\n"
    "4. Do not call skill_manage: you have no write access. Deciding is your "
    "whole job; the parent plugin owns application.\n\n"
    "Return EXACTLY one JSON object and nothing else:\n"
    '{"action": "create"|"patch"|"no_op", "kind": "skill"|"memory"|"prompt", '
    '"name": "...", "content": "...", "category": "...", "reason": "...", '
    '"expected_outcome": "...", "evidence": ["..."], '
    '"pattern_fingerprint": "..."}\n'
    "- create requires complete content for the new skill/memory/prompt note.\n"
    "- patch applies to skills only; give the name and your intended change — "
    "the full replacement is regenerated by the pipeline.\n"
    "- no_op requires a non-empty reason."
)


def _render_proposer_context(
    evidence_text: str,
    ...
    *,
    ...
    skills_list = _llm._render_overview(
        ...
        entry_kind="skill",
        max_entries=config.overview_max_entries(),
        max_chars=config.overview_max_chars(),
    )
    mems_list = _llm._render_overview(
        list(existing_memories or []),
        entry_kind="memory",
        max_entries=config.overview_max_entries(),
        max_chars=config.overview_max_chars(),
    )
    ...
    return (
        ...
        f"{patterns.format_patterns(error_patterns)}\n"
        "</untrusted_tool_result>\n\n"
        "=== USER CORRECTIONS (UNTRUSTED JSON) ===\n"
        f"{corrections}\n\n"
        "=== EXISTING SKILLS (name + description only) ===\n"
        f"{skills_list}\n\n"
        "=== EXISTING MEMORIES ===\n"
        f"{mems_list}\n"
        f"{unused_block}"
        f"{history_block}\n"
        "=== RECENT TRAJECTORY ===\n"
        f"{_llm._bounded_trajectory(evidence_text)}\n"
    )
...
def _propose_with_subagent(
    ...
    *,
    ...
    try:
        ...
        # Bare-import contexts (tests, offline runs) may have a stubbed
        # "agent" package. The request shape is the contract, not the class:
        # build a duck-typed request with identical fields so callers that
        # only read attributes keep working.
        logger.debug("SubagentLaunchRequest import unavailable: %s", exc)
    ...
    context = _render_proposer_context(
        evidence_text,
        existing_skills,
        existing_memories,
    ...
    try:
        ...
        timeout_seconds = 180
    terminal = lifecycle.wait(handle, timeout_seconds=timeout_seconds)
    if not getattr(terminal, "completed", False):
        try:
            lifecycle.cancel(handle, reason="refine proposer wait timed out")
        except Exception:
            logger.debug("Proposer subagent cancel failed", exc_info=True)
        return None, {
            "proposal_source": "structured",
            "subagent_fallback_reason": "subagent_timeout",
        }
    try:
...
def _refine_once(
    ...
    *,
    ...
    for _primary_attempt in range(_primary_attempt_limit):
        ...
        proposal = None
        if not _subagent_tried and config.proposer_subagent_enabled():
            # Preferred path: a read-only subagent that can OPEN skill bodies
            # (skills_list/skill_view) before deciding. Falls back to the
            # structured call when no parent turn is bound, the launch is
            # refused, the wait times out, or the answer cannot be parsed.
            _subagent_tried = True
            proposal, _subagent_meta = _propose_with_subagent(
                llm,
                evidence_text,
                list_skill_entries(),
