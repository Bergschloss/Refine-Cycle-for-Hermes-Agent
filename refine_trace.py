...
def emit_trace(trace: Dict[str, Any], *, journal_append=None) -> None:
    ...
    There is no redaction boundary here any more: the credential filter that
    used to run at this point was removed with the rest of the grammar, so a
    trace line carries what the pass actually saw. The log is local, under
    ``<hermes home>/logs/``, like every other plugin-owned artefact."""
    try:
