...
REFINE_SYSTEM_PROMPT = (
    ...
    "7a. A memory entry is read later by an agent that no longer remembers this "
    "trajectory, so it has to carry enough to act on: the condition that triggers it, "
    "and what to do. When the evidence shows the correction took several steps in a "
    "fixed order, state them in that order — a partial sequence teaches a partial fix. "
    "Keep it to the operation the evidence is about: never write it so it also stops or "
    "replaces a different operation that was not failing, and never so the corrected "
    "action becomes an unrelated one. These are requirements about meaning, not format: "
    "do not add literal labels, and do not exceed the length above to satisfy them — a "
    "longer entry is refused, so say it in one plain sentence or leave the lesson to "
    "kind=skill.\n"
