...
- journal, append-only, one record per attempt. A record a crash cut short (an unterminated last line that does not parse) is not part of it: reads skip it, and the next append moves it to `refine_journal.jsonl.torn`. Any other bad line still fails the journal closed.
...
1,320 tests, standard library `unittest`, no network. Running an individual test file directly will fail on imports; the runner sets the path.
