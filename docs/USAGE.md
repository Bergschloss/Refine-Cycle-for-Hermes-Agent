...
| A desktop app on this host has the plugin's desktop half, switched off (once per host) | `♾️ Refine Cycle has Update and Fix buttons in the Hermes desktop app. Turn them on once: Settings ▸ Plugins ▸ refine ▸ the Desktop switch.` |
...
**In the Hermes desktop app** the plugin's desktop half (`desktop/plugin.js`) puts one item in the status bar: `♾️ Refine Cycle 1.3.12 · working`, `· update available: 1.3.13` with an **Update** button, or `· not working` with a **Fix** button. While the app is in the background, the same event arrives as a system notification. On macOS it carries the same button; Electron puts no buttons in Windows toasts, so there a click on the notification itself starts the same action. Pressing it starts the work in the plugin's backend (Hermes stops waiting for a plugin command after 30 seconds, and an install takes longer), shows `♾️ Refine Cycle updated to 1.3.13. Restarting Hermes…` when it is done, and restarts the desktop backend, as Settings ▸ Restart backend does. Hermes loads a plugin's desktop half switched off; turn it on once on the Plugins page.
