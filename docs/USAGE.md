...
**A button inside the chat**, not only in the status bar: the desktop half claims the `::refine` transcript directive, so when the agent writes `::refine{}` on a line of its own the message shows a card with the same state and the same **Update** / **Fix** button, wired straight to the plugin's backend — no HTML file, no hidden turn, no model call for the press. Ask the agent to check on Refine Cycle and it can put the card in front of you.

What the plugin cannot do is put that card there by itself. Hermes renders a widget or a card only inside an **assistant** message (`apps/desktop/src/lib/transcript-directives.ts`), a plugin's own command output arrives as a `system` message drawn as plain text (`apps/desktop/src/components/assistant-ui/thread/system-message.tsx`), and there is no plugin API that posts to a chat: `hermes send` reaches messaging platforms only (`hermes send --list` on a desktop-only host lists none), and `inject_message` forges a *user* turn behind an `allow_gateway_injection` switch, which refine will not do. So on the desktop the proactive surface is the status bar, and the chat card is what the agent can hand you on request.
